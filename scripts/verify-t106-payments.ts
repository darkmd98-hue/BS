/**
 * T-106 Payment Gateway Integration — Dual-Lodge Live Verification
...
async function main() {
  console.log("================================================================================");
  console.log("     T-106 LIVE VERIFICATION: PAYMENT GATEWAY INTEGRATION (STRIPE & RAZORPAY)   ");
  console.log("================================================================================\n");
  ...
  // Check 1: Pinecrest Payment Gateways & Folio
  console.log("1. Checking /admin/payments under Pinecrest (Lodge A)...");
  let resA = await testRequest("pinecrest.localhost", "/admin/payments", cookieHeaderA);
  if (resA.status !== 200) {
    await delay(1000);
    resA = await testRequest("pinecrest.localhost", "/admin/payments", cookieHeaderA);
  }
  const hasTitleA = resA.body.includes("Payment Gateways") || resA.body.includes("Folio");
  const hasGatewaysA = resA.body.includes("Stripe") && resA.body.includes("Razorpay");
  const pass1 = resA.status === 200 && hasTitleA && hasGatewaysA;
  console.log(
    `   - Status: HTTP ${resA.status} (Title: ${hasTitleA}, Dual Gateways: ${hasGatewaysA}) -> ${
      pass1 ? "✅ PASS" : "❌ FAIL"
    }`
  );
  results.push(pass1);
  ...
  // Check 2: Lakeside Payment Gateways & Folio
  console.log("\n2. Checking /admin/payments under Lakeside (Lodge B)...");
  let resB = await testRequest("lakeside.localhost", "/admin/payments", cookieHeaderB);
  if (resB.status !== 200) {
    await delay(1000);
    resB = await testRequest("lakeside.localhost", "/admin/payments", cookieHeaderB);
  }
  const hasTitleB = resB.body.includes("Payment Gateways") || resB.body.includes("Folio");
  ...
  const resCross = await testRequest("lakeside.localhost", "/admin/payments", cookieHeaderA);
  ...
  const resAnon = await testRequest("pinecrest.localhost", "/admin/payments", "");
  const pass4 = resAnon.status === 307;
  console.log(`   - Status: HTTP ${resAnon.status} (Expected: 307 Redirect) -> ${pass4 ? "✅ PASS" : "❌ FAIL"}`);
  results.push(pass4);
  await delay(500);

  // Check 5: Webhook Ingestion Endpoint Probe
  console.log("\n5. Webhook Ingestion Health Probe (GET /api/webhooks/payments)...");
  const resWebhook = await testRequest("pinecrest.localhost", "/api/webhooks/payments", "");
  const pass5 = resWebhook.status === 200 && resWebhook.body.includes("active");
  console.log(`   - Status: HTTP ${resWebhook.status} (Body: ${resWebhook.body}) -> ${pass5 ? "✅ PASS" : "❌ FAIL"}`);
  results.push(pass5);
  ...
  if (passed < results.length) {
    console.error("❌ Some T-106 verification checks failed!");
    process.exit(1);
  }
  console.log("🎉 ALL T-106 PAYMENT GATEWAY CHECKS PASSED!");
  console.log("================================================================================");
}
