/**
 * T-105 Advanced RBAC — Dual-Lodge Live Verification
...
async function main() {
  console.log("================================================================================");
  console.log("     T-105 LIVE VERIFICATION: ADVANCED RBAC & PERMISSION MATRIX                 ");
  console.log("================================================================================\n");
  ...
  // Check 1: Pinecrest Roles & Permissions
  console.log("1. Checking /admin/roles under Pinecrest (Lodge A)...");
  let resA = await testRequest("pinecrest.localhost", "/admin/roles", cookieHeaderA);
  if (resA.status !== 200) {
    await delay(1000);
    resA = await testRequest("pinecrest.localhost", "/admin/roles", cookieHeaderA);
  }
  const hasRbacA = resA.body.includes("Access Control") || resA.body.includes("Permissions Matrix");
  const hasRolesA = resA.body.includes("Super Admin") || resA.body.includes("General Manager");
  const pass1 = resA.status === 200 && hasRbacA && hasRolesA;
  console.log(`   - Status: HTTP ${resA.status} (RBAC Title: ${hasRbacA}, System Roles: ${hasRolesA}) -> ${pass1 ? "✅ PASS" : "❌ FAIL"}`);
  results.push(pass1);
  await delay(500);

  // Check 2: Lakeside Roles & Permissions
  console.log("\n2. Checking /admin/roles under Lakeside (Lodge B)...");
  let resB = await testRequest("lakeside.localhost", "/admin/roles", cookieHeaderB);
  if (resB.status !== 200) {
    await delay(1000);
    resB = await testRequest("lakeside.localhost", "/admin/roles", cookieHeaderB);
  }
  const hasRbacB = resB.body.includes("Access Control") || resB.body.includes("Permissions Matrix");
  const pass2 = resB.status === 200 && hasRbacB;
  console.log(`   - Status: HTTP ${resB.status} (RBAC Title: ${hasRbacB}) -> ${pass2 ? "✅ PASS" : "❌ FAIL"}`);
  results.push(pass2);
  await delay(500);
  ...
  const resCross = await testRequest("lakeside.localhost", "/admin/roles", cookieHeaderA);
  ...
  const resAnon = await testRequest("pinecrest.localhost", "/admin/roles", "");
  ...
  if (passed < results.length) {
    console.error("❌ Some T-105 verification checks failed!");
    process.exit(1);
  }
  console.log("🎉 ALL T-105 ADVANCED RBAC CHECKS PASSED!");
  console.log("================================================================================");
}
